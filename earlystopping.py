...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False):
        ...
        self.early_stop = False
        self.val_loss_min = np.Inf
        self.save_model = False  # モデルを保存するかどうかのフラグ
    ...
    def __call__(self, val_loss, model, epoch, save_path):
        ...
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, epoch, save_path)
            self.save_model = True  # モデル保存フラグをTrueに設定
        elif score < self.best_score:
            self.counter += 1
            self.save_model = False  # モデル保存フラグをFalseに設定
    ...
    def save_checkpoint(self, val_loss, model, epoch, save_path):
        ...
        torch.save(
            model, os.path.join(save_path, f"checkpoint_{epoch}_{val_loss:.6f}.pth.tar")
        )
        self.val_loss_min = val_loss
